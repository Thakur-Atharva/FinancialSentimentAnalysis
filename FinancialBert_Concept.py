...
def sent_score(list_dict):
    sent = 0
    for dict in list_dict:
        if dict['label'] == 'positive':
            sent += dict['score']
        elif dict['label'] == 'negative':
            sent -= dict['score']
    return sent
...
def get_sentiment(text):
    ...
    for i in text.split('. '):
        for j in i.split('and '):
            print(j + '\n')
            output = requests.post(API_URL, headers=headers, json={"inputs": f"{j}"}).json()
            count += 1
            sentiment += sent_score(output)
            print(j, sentiment, '\n')
    return sentiment/count


# print(get_sentiment(testA))